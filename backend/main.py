...
def register_form(
    request: Request, db: Session = Depends(get_db), session: str = Cookie(None)
):
...
async def register_from_event(
    ...
    if not name or not email:
        return RedirectResponse(
            url=f"/event/{event_id}?error=Name+and+email+are+required", status_code=303
        )
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/event/{event_id}?error={str(e).replace(' ', '+')}", status_code=303
        )
    ...
    return RedirectResponse(
        url=f"/registration-success/{registration.id}", status_code=303
    )
...
def registration_success(
    ...
    event = (
        db.query(models.Event).filter(models.Event.id == registration.event_id).first()
    )
...
def login(
    ...
    admin = (
        ...
        .filter(models.Admin.username == username, models.Admin.password == password)
        .first()
    )
...
def dashboard(
    ...
    most_popular = (
        max(event_counts, key=lambda x: x["count"])
        if event_counts
        else {"id": 0, "title": "N/A", "count": 0}
    )
...
def create_event(
    ...
    try:
        ...
        return RedirectResponse(
            url="/dashboard?error=" + str(e).replace(" ", "+"), status_code=303
        )
    ...
    return RedirectResponse(
        url="/dashboard?success=Event+created+successfully!", status_code=303
    )
...
def delete_event(
    event_id: int, db: Session = Depends(get_db), session: str = Cookie(None)
):
    if session != ADMIN_SESSION_TOKEN:
...
def edit_event_page(
    ...
    if not event:
        return RedirectResponse(url="/dashboard", status_code=303)
    return templates.TemplateResponse(
        "edit_event.html", {"request": request, "event": event}
    )
...
def edit_event(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/edit/{event_id}?error={str(e).replace(' ', '+')}", status_code=303
        )
    ...
    db.commit()
    return RedirectResponse(
        url="/dashboard?success=Event+updated+successfully!", status_code=303
    )
...
def create_admin(db: Session = Depends(get_db)):
    existing = (
        db.query(models.Admin).filter(models.Admin.username == ADMIN_USERNAME).first()
    )
    if existing:
